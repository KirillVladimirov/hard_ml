...
class GaussianKernel(torch.nn.Module):
    ...
    def forward(self, x):
        sigma = torch.FloatTensor([self.sigma])
        return torch.exp(-torch.square(torch.sub(x, self.mu)) / (2 * torch.square(sigma)))
...
class KNRM(torch.nn.Module):
    ...
    def _get_kernels_layers(self) -> torch.nn.ModuleList:
        ...
        start_point = -1 + 1 / (self.kernel_num - 1)
        step_size = 2 / (self.kernel_num - 1)
        for i in range(self.kernel_num-1):
            mu = start_point + step_size*i
            kernels.append(GaussianKernel(mu=mu, sigma=self.sigma))
        kernels.append(GaussianKernel(mu=1.0, sigma=self.exact_sigma))
        return kernels

    def _get_mlp(self) -> torch.nn.Sequential:
        layers_schema = [self.kernel_num] + self.out_layers + [1]
        mlp = [
            torch.nn.Sequential(
                torch.nn.Linear(input_number, output_number),
                torch.nn.ReLU())
            if output_number != 1 else
            torch.nn.Linear(input_number, output_number)
            for input_number, output_number in zip(layers_schema, layers_schema[1:])
        ]
    ...
    def _get_matching_matrix(self, query: torch.Tensor, doc: torch.Tensor) -> torch.FloatTensor:
        # shape = [Batch, Query, Embeddings]
        embed_query = self.embeddings(query)
        # shape = [Batch, Doc, Embeddings]
        embed_doc = self.embeddings(doc)
        # shape = [Batch, Query, Doc]
...
class Solution:
    ...
    def simple_preproc(self, inp_str: str) -> List[str]:
        """
            ...
            а в качестве токенизации (разбиения предложения на слова или их усеченную версию — токены)
            необходимо использовать метод nltk.word_tokenize
        """
        return nltk.word_tokenize(self.handle_punctuation(inp_str.strip().lower()))

    def _filter_rare_words(self, vocab: Dict[str, int], min_occurancies: int) -> Dict[str, int]:
        return {key: count for key, count in vocab.items() if count >= min_occurancies}

    def get_all_tokens(self, list_of_df: List[pd.DataFrame], min_occurancies: int) -> List[str]:
        list_of_texts = []
        for df in list_of_df:
            list_of_texts.extend(df["text_left"])
            list_of_texts.extend(df["text_right"])
        list_of_texts = list(set(list_of_texts))
        cnt = Counter()
        for text in list_of_texts:
            cnt.update(self.simple_preproc(text))
        trunked_vocab = self._filter_rare_words(cnt, min_occurancies)
        return list(trunked_vocab.keys())
    ...
    def create_glove_emb_from_file(self, file_path: str, inner_keys: List[str],
        ...
        input_dim, out_dim = len(inner_keys), len(list(embeddings.values())[0])
        emb_matrix = np.zeros((input_dim, out_dim))
        vocab = {}
        unk_words = []
        unk_words.append(inner_keys[0])
        vocab[inner_keys[0]] = 0
        for i, word in enumerate(inner_keys[1:]):
            vocab[word] = i+1
            if word in embeddings:
                emb_matrix[i+1] = embeddings[word]
            else:
                emb_matrix[i+1] = np.random.uniform(-rand_uni_bound, rand_uni_bound, size=out_dim)
